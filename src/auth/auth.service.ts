...
export class AuthService {
  ...
  async invalidAccessToken(user: any): Promise<boolean> {
    ...
    for (const tokenDoc of tokenDatas) {
  ...
  async refreshToken(body: RefreshJwtDto): Promise<ApiResponse> {
    ...
    if (tokenData?.uuid) {
      ...
      if (refreshTokenData.createdAt > expiresDate) {
        await this.refreshTokenRepository.delete(refreshTokenData.uuid);
      ...
      if (existingDeviceData?.uuid) {
        await this.userDeviceRepository.updateById(
          {
            accessToken: token,
          },
          existingDeviceData?.uuid,
        );
      }
  ...
  async userLogin(body: UserSignInDTO, req: Request): Promise<ApiResponse> {
    ...
    const payload = { id: checkIfExists.uuid };
    ...
    this.invalidAccessToken(userDetails.uuid);
